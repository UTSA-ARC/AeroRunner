...
vector<fixed> Get_Raw_Accel() {

  vector<fixed> result;

  result.push_back(RawAccX = (Wire.read() << 8 | Wire.read())); // Raw X-axis value
  result.push_back(RawAccY = (Wire.read() << 8 | Wire.read())); // Raw Y-axis value
  result.push_back(RawAccZ = (Wire.read() << 8 | Wire.read())); // Raw Z-axis value

  return result;

}

vector<fixed> Accel(fixed raw_x, fixed raw_y, fixed raw_z) {
  
  vector<fixed> normalized_accel = {
                                          raw_x / ALSB_Sensitivity, 
                                          raw_y / ALSB_Sensitivity, 
                                          raw_z / ALSB_Sensitivity
                                        };
  return normalized_accel;

}

vector<fixed> Accel(vector<fixed> raw_accel) {
  
  vector<fixed> normalized_accel = {
                                          raw_accel.at(0) / ALSB_Sensitivity, 
                                          raw_accel.at(1) / ALSB_Sensitivity, 
                                          raw_accel.at(2) / ALSB_Sensitivity
                                        };
  return normalized_accel;
...
void loop() {
  ...
  vector<fixed> accel = Accel(Get_Raw_Accel());
  ...
  short RawGyroX = (short) ((Wire.read() << 8 | Wire.read()) * 1000);
  short RawGyroY = (short) ((Wire.read() << 8 | Wire.read()) * 1000);
  short RawGyroZ = (short) ((Wire.read() << 8 | Wire.read()) * 1000);
  
  short GyroX = RawGyroX / GLSB_Sensitivity;
  short GyroY = RawGyroY / GLSB_Sensitivity;
  short GyroZ = RawGyroZ / GLSB_Sensitivity;
