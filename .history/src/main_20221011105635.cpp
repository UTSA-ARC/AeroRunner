...
vector<int> Get_Raw_Accel() {
  ...
  result.push_back(RawAccX = (Wire.read() << 8 | Wire.read()) * 10000); // Raw X-axis value
  result.push_back(RawAccY = (Wire.read() << 8 | Wire.read()) * 10000); // Raw Y-axis value
  result.push_back(RawAccZ = (Wire.read() << 8 | Wire.read()) * 10000); // Raw Z-axis value
...
void loop() {
  ...
  vector<int> accel = Normalized_Accel(Get_Raw_Accel());
